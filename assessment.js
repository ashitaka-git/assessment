...
assessmentButton.addEventListener( //イベント検知設定の追加
  ...
  paragraph.innerText = result //pタグの内側のテキストの設定
  resultDivision.appendChild(paragraph)//divタグの子要素としてpタグの作成
...
userNameInput.addEventListener( //イベント検知設定の追加
  'keydown', //enterイベント
  (event) => { //enterされると実行される
    if (event.code === 'Enter'){
